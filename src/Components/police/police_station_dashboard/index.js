...
import { Dashboard, Person, Message, Settings, Notifications, Logout, Brightness4, Brightness7, VerifiedUser, Warning, Search, History, CheckCircle, Gavel } from '@mui/icons-material';
import './index.css';

const actions = [
  { label: 'Raise Alert', icon: <Warning color="primary" />, onClick: () => alert('Raise Alert') },
  { label: 'Search Case', icon: <Search color="primary" />, onClick: () => alert('Search Case') },
];

const analytics = [
  { label: 'Cases Handled', value: 21, icon: <Gavel color="primary" /> },
  { label: 'Alerts Raised', value: 4, icon: <CheckCircle color="success" /> },
];

const recentActivity = [
  { type: 'Case', desc: 'Closed theft case', time: '3h ago' },
  { type: 'Alert', desc: 'Raised city-wide alert', time: '1d ago' },
  { type: 'Case', desc: 'Investigated missing person', time: '2d ago' },
];
...
const PoliceDashboard = ({ onNavigate }) => {
  const [user, setUser] = useState({ name: '', role: 'Police Officer', verified: false });
  const [darkMode, setDarkMode] = useState(false);

  useEffect(() => {
    const u = JSON.parse(localStorage.getItem('user'));
    if (u) setUser({ ...u, role: 'Police Officer', verified: u.verified });
  }, []);
  ...
  return (
    <Box sx={{ display: 'flex', bgcolor: darkMode ? '#181c24' : '#f4f6fa', minHeight: '100vh' }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: { xs: 1, sm: 3 } }}>
        {/* AppBar */}
        <AppBar position="static" color={darkMode ? 'default' : 'primary'} elevation={0} sx={{ bgcolor: darkMode ? '#23272f' : '#e67e22', color: '#fff', borderRadius: 2, mb: 2 }}>
          <Toolbar sx={{ justifyContent: 'space-between' }}>
            <Typography variant="h6" noWrap component="div">Police Dashboard</Typography>
            <Box>
        ...
        <Card sx={{ mb: 3, p: 2, display: 'flex', alignItems: 'center', bgcolor: darkMode ? '#23272f' : '#fff' }}>
          ...
          <Box>
            <Typography variant="h6">{user.name || 'Police Officer'}</Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Typography variant="body2" color="text.secondary">{user.role}</Typography>
              {user.verified ? <VerifiedUser color="success" fontSize="small" titleAccess="Verified" /> : <VerifiedUser color="warning" fontSize="small" titleAccess="Pending Verification" />}
            </Box>
            <Typography variant="caption" color={user.verified ? 'success.main' : 'warning.main'}>
              {user.verified ? 'Verified Police Officer' : 'Verification Pending'}
            </Typography>
          </Box>
        </Card>
